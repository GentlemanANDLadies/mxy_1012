...
var turnWheel = {
	...
	colors: [], //转盘奖品区块对应背景颜色
	outsideRadius: 174, //转盘外圆的半径
	textRadius: 143, //转盘奖品位置距离圆心的距离
	insideRadius: 50, //转盘内圆的半径
	startAngle: 0, //开始角度
	...
	var imgLl1 = new Image();
	imgLl1.src = "~/../images/teny.png";

	var imgLl2 = new Image();
	imgLl2.src = "~/../images/by.png";

	var imgLl3 = new Image();
	imgLl3.src = "~/../images/fivey.png";
...
$(document).ready(function() {
	...
	*/
	...
	turnWheel.rewardNames = ["京东购物卡50元", "谢谢参与","话费10元", "谢谢参与","京东购物卡1000元", "谢谢参与",]
	turnWheel.colors = ["#ffc200", "#fff","#ffc200", "#fff","#ffc200", "#fff"];
    ...
    //             } else {
    //             	$(".tips").fadeIn();
    //             	// $("#tip").html('获得<span style="font-size:.4rem;color:#fe533c;font-weight:900;">' + tip + '</span>流量包<br/>多跟老爸聊一聊吧!');
    //             	// $("#tip").html('恭喜获得<span style="font-size:.4rem;color:#fe533c;font-weight:900;">' + tip + '</span><br/>赶紧送到老爸手里吧!');
    //             	// $("#tip").html('恭喜获得<span style="font-size:.44rem;color:#fe533c;font-weight:900;">' + tip + '</span><br/>多跟老爸唠一唠吧!');
    //                 turnWheel.bRotate = !turnWheel.bRotate;
    //             }
			}
		});
	...
	console.log("rewardNames["+random+"]");
		if (random==1 || random==3 || random==6 ) {
			setTimeout(function(){
				...
				setTimeout(function(){
				window.location.href='getJp50.html';
			},7000)			
		}else if(random==2){
			setTimeout(function(){
				window.location.href='getJp10.html';
			},7000)	
		}else if(random==4){
		setTimeout(function(){
				window.location.href='getJp1000.html';
			},7000)
		}
	return random;
}
	...
	for(var index = 0; index < turnWheel.rewardNames.length; index++) {
		...
		if(rewardName.indexOf("M") > 0) { //查询是否包含字段 流量包
			...
			for(var j = 0; j < rewardNames.length; j++) {
				ctx.font = (j == 0) ? '100 16px Microsoft YaHei' : '16px Microsoft YaHei';
				if(j == 0) {
		...
		//添加对应图标
		if(rewardName.indexOf("谢谢参与") >= 0) {
			imgSorry.onload = function() {
				ctx.drawImage(imgSorry, -20, 10);
			};
			ctx.drawImage(imgSorry, -20, 10);
		}else if(rewardName.indexOf("话费10元") >= 0) {
			// 注意，这里要等到img加载完成才能绘制
			imgLl1.onload = function() {
				ctx.drawImage(imgLl1, -30, 20);
			};
			ctx.drawImage(imgLl1, -30, 20);
		} else if(rewardName.indexOf("京东购物卡1") >= 0) {
			// 注意，这里要等到img加载完成才能绘制
			imgLl2.onload = function() {
				ctx.drawImage(imgLl2, -30, 20);
			};
			ctx.drawImage(imgLl2, -30, 20);

		} else if(rewardName.indexOf("京东购物卡5") >= 0){
			// 注意，这里要等到img加载完成才能绘制
			imgLl3.onload = function() {
				ctx.drawImage(imgLl3, -30, 20);
			};
			ctx.drawImage(imgLl3, -30, 20);
		}
